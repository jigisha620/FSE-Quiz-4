...
class BasicRabbitListenerTest {
    ...
    @Before
    fun before() {
        BasicRabbitConfiguration("test-exchange", "test-queue").setUp()
    ...
    fun listener() {
        ...
        factory.newConnection().use { connection ->
            connection.createChannel().use { channel ->
                ...
                channel.basicPublish("", "test-queue", MessageProperties.BASIC, body)
            }
        }
        ...
        factory.newConnection().use { connection ->
            connection.createChannel().use { channel ->
                (1..50).map {
                    ...
                    channel.basicPublish("", "test-queue", MessageProperties.PERSISTENT_BASIC, body)
                }
            }
        }
